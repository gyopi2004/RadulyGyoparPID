...
namespace Framework.ViewModel
{
    public class MenuCommands : BaseVM
    {
        ...
        public ICommand LinearImageCommand
        {
            get
            {
                if (_linearImageCommand == null)
                    _linearImageCommand = new RelayCommand(LinearImage);
                return _linearImageCommand;
            }
        }

        private void LinearImage(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Please add an image!");
            ...
            List<double> values = window.GetValues();
            float alpha=(float)values[0];
            float beta = (float)values[1];

            byte[] lut = PointwiseOperations.CreareLinearOpLUT(alpha, beta);
            ClearProcessedCanvas(parameter as Canvas);
            ...
            {
                GrayProcessedImage = Tools.Convert(ColorInitialImage);
                GrayProcessedImage = PointwiseOperations.ApplyLut(GrayProcessedImage, lut);
                ProcessedImage = Convert(GrayProcessedImage);
            }
        }
        #endregion
